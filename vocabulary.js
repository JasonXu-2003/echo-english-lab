(() => {
  ...
  const ratingKeys={again:'1',hard:'2',good:'Space'};
  const btn=(action,text,cls='')=>`<button type="button" class="v-btn ${cls}" data-v-action="${action}"${ratingKeys[action]?` aria-keyshortcuts="${ratingKeys[action]}" title="快捷键 ${action==='good'?'空格':ratingKeys[action]}"`:''}>${text}</button>`;
  ...
  function init(adapter) {
    ...
    document.addEventListener('keydown',e=>{
      ...
      if(current&&revealed&&!rated&&['1','2',' '].includes(e.key)) {
        e.preventDefault();rate({'1':'again','2':'hard',' ':'good'}[e.key]);return;
      }
